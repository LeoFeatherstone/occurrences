...
names_xml <- names_xml[-grep(names_xml, pattern='pruned')]
...
for (i in 1:length(names_xml)){
	...
	bdsky_xml[[i]] <- c(bdsky_xml[[i]], xml[[i]][(1+max(grep(xml[[i]], pattern='map>'))):grep(xml[[i]], pattern='<distribution id="BirthDeathSkySerial')-1])
